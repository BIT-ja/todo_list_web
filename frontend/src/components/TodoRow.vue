  ...
  <van-swipe-cell>
    ...
    >
      ...
      <div class="todo-item__content">
        ...
        <div v-if="todo.content" class="todo-item__desc">{{ todo.content }}</div>
        <div v-if="todo.due_at || todo.location || todo.priority > 0" class="todo-item__meta">
          <span v-if="todo.due_at" class="todo-item__due" :class="{ 'todo-item__due--overdue': isOverdue }">
            {{ formatDate(todo.due_at) }}
          </span>
          <span v-if="todo.location" class="todo-item__location">
...
const isOverdue = computed(() => {
  ...
  return new Date(props.todo.due_at) < new Date()
})
